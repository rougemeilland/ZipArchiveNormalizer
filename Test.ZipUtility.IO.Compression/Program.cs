...
namespace Test.ZipUtility.Compression
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var size = (ulong)testFile.Length;
            foreach (var compressionMethod in compressionMethods)
            {
                using (var ms = new MemoryStream())
                {
                    using (var inputStream = testFile.OpenRead().AsInputByteStream())
                    {
                        using (var outputStream = compressionMethod.GetOutputStream(ms.AsOutputByteStream(true), size))
                        {
                            inputStream.CopyTo(outputStream, count => Console.Write("."));
                        }
                    }
                    var packedSize = (ulong)ms.Position;
                    ms.Seek(0, SeekOrigin.Begin);
                    using (var inputStream1 = testFile.OpenRead().AsInputByteStream())
                    {
                        using (var inputStream2 = compressionMethod.GetInputStream(ms.AsInputByteStream(true), size))
                        {
