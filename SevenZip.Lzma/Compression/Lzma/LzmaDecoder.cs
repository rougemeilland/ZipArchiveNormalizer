...
namespace SevenZip.Compression.Lzma
{
    ...
    public class LzmaDecoder : ICoder, ISetDecoderProperties // ,System.IO.Stream
    {
        class LenDecoder
        {
            ...
            BitTreeDecoder[] m_LowCoder = new BitTreeDecoder[LzmaCoder.kNumPosStatesMax];
            BitTreeDecoder[] m_MidCoder = new BitTreeDecoder[LzmaCoder.kNumPosStatesMax];
            BitTreeDecoder m_HighCoder = new BitTreeDecoder(LzmaCoder.kNumHighLenBits);
        ...
        BitDecoder[] m_IsMatchDecoders = new BitDecoder[LzmaCoder.kNumStates << LzmaCoder.kNumPosStatesBitsMax];
        BitDecoder[] m_IsRepDecoders = new BitDecoder[LzmaCoder.kNumStates];
        BitDecoder[] m_IsRepG0Decoders = new BitDecoder[LzmaCoder.kNumStates];
        BitDecoder[] m_IsRepG1Decoders = new BitDecoder[LzmaCoder.kNumStates];
        BitDecoder[] m_IsRepG2Decoders = new BitDecoder[LzmaCoder.kNumStates];
        BitDecoder[] m_IsRep0LongDecoders = new BitDecoder[LzmaCoder.kNumStates << LzmaCoder.kNumPosStatesBitsMax];

        BitTreeDecoder[] m_PosSlotDecoder = new BitTreeDecoder[LzmaCoder.kNumLenToPosStates];
        BitDecoder[] m_PosDecoders = new BitDecoder[LzmaCoder.kNumFullDistances - LzmaCoder.kEndPosModelIndex];

        BitTreeDecoder m_PosAlignDecoder = new BitTreeDecoder(LzmaCoder.kNumAlignBits);
            ...
            Int64 inSize, Int64 outSize, ICodeProgress progress)
        {
            ...
            {
                if (m_IsMatchDecoders[state.Index << LzmaCoder.kNumPosStatesBitsMax].Decode(m_RangeDecoder) != 0)
            ...
            while (nowPos64 < outSize64)
            {
                    ...
                    // while(nowPos64 < next)
                {
                    ...
                    if (m_IsMatchDecoders[(state.Index << LzmaCoder.kNumPosStatesBitsMax) + posState].Decode(m_RangeDecoder) == 0)
                    {
                        byte b;
                    ...
                    else
                    {
                        ...
                        if (m_IsRepDecoders[state.Index].Decode(m_RangeDecoder) == 1)
                        {
                            ...
                            {
                                if (m_IsRep0LongDecoders[(state.Index << LzmaCoder.kNumPosStatesBitsMax) + posState].Decode(m_RangeDecoder) == 0)
                                {
