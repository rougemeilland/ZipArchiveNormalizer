...
namespace SevenZip.Compression.Lzma
{
    ...
    public class LzmaEncoder : ICoder, ISetCoderProperties, IWriteCoderProperties
    {
        ...
        LzmaCoder.State _state = new LzmaCoder.State();
        Byte _previousByte;
        UInt32[] _repDistances = new UInt32[LzmaCoder.kNumRepDistances];
        ...
        const UInt32 kNumLenSpecSymbols = LzmaCoder.kNumLowLenSymbols + LzmaCoder.kNumMidLenSymbols;

        class LenPriceTableEncoder : LenEncoder
        {
            UInt32[] _prices = new UInt32[LzmaCoder.kNumLenSymbols << LzmaCoder.kNumPosStatesBitsEncodingMax];
            UInt32 _tableSize;
            UInt32[] _counters = new UInt32[LzmaCoder.kNumPosStatesEncodingMax];
        ...
        RangeCoder.BitEncoder[] _isMatch = new RangeCoder.BitEncoder[LzmaCoder.kNumStates << LzmaCoder.kNumPosStatesBitsMax];
        RangeCoder.BitEncoder[] _isRep = new RangeCoder.BitEncoder[LzmaCoder.kNumStates];
        RangeCoder.BitEncoder[] _isRepG0 = new RangeCoder.BitEncoder[LzmaCoder.kNumStates];
        RangeCoder.BitEncoder[] _isRepG1 = new RangeCoder.BitEncoder[LzmaCoder.kNumStates];
        RangeCoder.BitEncoder[] _isRepG2 = new RangeCoder.BitEncoder[LzmaCoder.kNumStates];
        RangeCoder.BitEncoder[] _isRep0Long = new RangeCoder.BitEncoder[LzmaCoder.kNumStates << LzmaCoder.kNumPosStatesBitsMax];

        RangeCoder.BitTreeEncoder[] _posSlotEncoder = new RangeCoder.BitTreeEncoder[LzmaCoder.kNumLenToPosStates];
        
        RangeCoder.BitEncoder[] _posEncoders = new RangeCoder.BitEncoder[LzmaCoder.kNumFullDistances - LzmaCoder.kEndPosModelIndex];
        RangeCoder.BitTreeEncoder _posAlignEncoder = new RangeCoder.BitTreeEncoder(LzmaCoder.kNumAlignBits);
        ...
        UInt32[] _matchDistances = new UInt32[LzmaCoder.kMatchMaxLen * 2 + 2];
        ...
        UInt32[] _posSlotPrices = new UInt32[1 << (LzmaCoder.kNumPosSlotBits + LzmaCoder.kNumLenToPosStatesBits)];
        UInt32[] _distancesPrices = new UInt32[LzmaCoder.kNumFullDistances << LzmaCoder.kNumLenToPosStatesBits];
        UInt32[] _alignPrices = new UInt32[LzmaCoder.kAlignTableSize];
        ...
        UInt32 GetRepLen1Price(LzmaCoder.State state, UInt32 posState)
        {
            return _isRepG0[state.Index].GetPrice0() +
                    _isRep0Long[(state.Index << LzmaCoder.kNumPosStatesBitsMax) + posState].GetPrice0();
        }

        UInt32 GetPureRepPrice(UInt32 repIndex, LzmaCoder.State state, UInt32 posState)
        {
            UInt32 price;
            if (repIndex == 0)
            {
        ...
        UInt32 GetRepPrice(UInt32 repIndex, UInt32 len, LzmaCoder.State state, UInt32 posState)
        {
        ...
        UInt32[] reps = new UInt32[LzmaCoder.kNumRepDistances];
        UInt32[] repLens = new UInt32[LzmaCoder.kNumRepDistances];
        ...
        void WriteEndMarker(UInt32 posState)
        {
            ...
            _isMatch[(_state.Index << LzmaCoder.kNumPosStatesBitsMax) + posState].Encode(_rangeEncoder, 1);
        ...
        public void CodeOneBlock(out Int64 inSize, out Int64 outSize, out bool finished)
        {
            ...
            if (nowPos64 == 0)
            {
                ...
                _isMatch[(_state.Index << LzmaCoder.kNumPosStatesBitsMax) + posState].Encode(_rangeEncoder, 0);
                _state.UpdateChar();
        ...
        UInt32[] tempPrices = new UInt32[LzmaCoder.kNumFullDistances];
        ...
        public void SetCoderProperties(CoderProperties properties)
        {
            ...
            _matchFinderType =
                Enum.GetValues(typeof(EMatchFinderType))
                .Cast<EMatchFinderType>()
                .Where(value => string.Equals(value.ToString(), key, StringComparison.OrdinalIgnoreCase))
                .Concat(new[] { (EMatchFinderType)Int32.MinValue })
                .First();
            if (_matchFinderType == (EMatchFinderType)Int32.MinValue)
                throw new InvalidParamException();
