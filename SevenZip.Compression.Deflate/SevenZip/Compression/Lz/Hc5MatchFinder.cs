...
{
    class Hc5MatchFinder
        : HcMatchFinder
    {
        private readonly UInt32[] _fixedHash2;
        private readonly UInt32[] _fixedHash3;

        public Hc5MatchFinder(UInt32 historySize, UInt32 keepAddBufferBefore, UInt32 matchMaxLen, UInt32 keepAddBufferAfter, UInt32 matchFinderCycles)
        ...
        public override UInt32 GetMatches(IBasicInputByteStream inStream, Span<UInt32> distances)
        {
            ...
            _fixedHash3[h3] = Pos;
            Hash[hv] = Pos;
            var pos2 = CyclicBufferSize.Minimum(Pos);
            var maxLen = 4U;
            while (true)
            {
                if (distance2 < pos2 && (BufferPointer - distance2)[0] == BufferPointer[0])
                {
                    ...
                    else if (distance3 < pos2 && (BufferPointer - distance3)[0] == BufferPointer[0])
                    {
                ...
                {
                    if ((BufferPointer + maxLen - distance2)[0] != BufferPointer[maxLen])
                        break;
                    ++maxLen;
                }
