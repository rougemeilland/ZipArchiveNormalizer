...
namespace ZipArchiveNormalizer
{
    static class ByteArrayExtensions
    {
        ...
        public static UInt32 CalculateCRC32(this byte[] buffer, CRC32Algorithm algorithm = CRC32Algorithm.Default)
        {
            return buffer.CalculateCRC32(0, buffer.Length, algorithm);
        }

        public static UInt32 CalculateCRC32(this byte[] buffer, int offset, CRC32Algorithm algorithm = CRC32Algorithm.Default)
        {
            return buffer.CalculateCRC32(offset, buffer.Length - offset, algorithm);
        }

        public static UInt32 CalculateCRC32(this byte[] buffer, int offset, int count, CRC32Algorithm algorithm = CRC32Algorithm.Default)
        {
            UInt32 value;
            switch (algorithm)
            {
                case CRC32Algorithm.Default:
                    ...
                    for (var i = 0; i < count; i++)
                        value = _crcTable1[(value ^ buffer[offset + i]) & 0xff] ^ (value >> 8);
                    return value ^ 0xffffffffU;
                case CRC32Algorithm.TYpe2:
                    value = 0xffffffffU;
        ...
        public static bool EqualsByteArray(this byte[] array1, byte[] array2)
        {
            ...
            else
            {
                return
                    ...
                    array1
                        .Zip(array2, (byte1, byte2) => new { byte1, byte2 })
                        .All(item => item.byte1 == item.byte2);
            }
        }
        ...
        public static Encoding GuessWhichEncoding(this byte[] bytes)
        {
            ...
            {
                var b1 = bytes[i];
                if (b1 <= 0x06 || b1.IsAnyOf(0x7f, 0xff))
                {
                    isBinary = true;
            ...
            for (int i = 0; i < len - 1; i++)
            {
                var b1 = bytes[i];
                var b2 = bytes[i + 1];
                if ((b1.IsBetween(0x81, 0x9f) || b1.IsBetween(0xe0, 0xfc)) &&
                    (b2.IsBetween(0x40, 0x7e) || b2.IsBetween(0x80, 0xfc)))
                {
                    //SJIS_C
                    count_shift_jis += 2;
                    i++;
            ...
            for (int i = 0; i < len - 1; i++)
            {
                var b1 = bytes[i];
                var b2 = bytes[i + 1];
                if (b1.IsBetween(0xa1, 0xfe) && b2.IsBetween(0xa1, 0xfe) ||
                    b1 == 0x8e && b2.IsBetween(0xa1, 0xdf))
                {
                    //EUC_C
                    //EUC_KANA
                    count_euc += 2;
                ...
                {
                    if (b1 == 0x8f &&
                        b2.IsBetween(0xa1, 0xfe) &&
                        bytes[i + 2].IsBetween(0xa1, 0xfe))
                    {
                        //EUC_0212
                        count_euc += 3;
                        i += 2;
            ...
            for (int i = 0; i < len - 1; i++)
            {
                var b1 = bytes[i];
                var b2 = bytes[i + 1];
                if (b1.IsBetween(0xc0, 0xdf) &&
                    b2.IsBetween(0x80, 0xbf))
                {
                    //UTF8
                    count_utf8 += 2;
                    i++;
                ...
                {
                    if (b1.IsBetween(0xe0, 0xef) &&
                        b2.IsBetween(0x80, 0xbf) &&
                        bytes[i + 2].IsBetween(0x80, 0xbf))
                    {
                        //UTF8
                        count_utf8 += 3;
                        i += 2;
        ...
        private static bool IsBetween(this byte data, byte lowerByte, byte upperBite)
        {
            return data >= lowerByte && data <= upperBite;
        }

        private static bool IsAnyOf(this byte data, byte data1, byte data2)
        {
            return data == data1 || data == data2;
        }

        private static bool IsAnyOf(this byte data, byte data1, byte data2, byte data3)
        {
            return data == data1 || data == data2 || data == data3;
        }

        private static bool IsAnyOf(this byte data, byte data1, byte data2, byte data3, byte data4)
        {
            return data == data1 || data == data2 || data == data3 || data == data4;
        }
    }
}
